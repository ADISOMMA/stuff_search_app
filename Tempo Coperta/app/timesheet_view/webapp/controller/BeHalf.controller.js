sap.ui.define([
    ...
    function (BaseController, Component, ODataModel, DateFormat, Text, JSONModel, Dialog, Button, Fragment, MessageToast, MessageBox, History, syncStyleClass, UnifiedLibrary, DateTypeRange, UI5Date) {
        "use strict";
        ...
        return BaseController.extend("com.lobra.timesheetview.controller.BeHalf", {
            targetName: "BeHalf",

            onInit: async function () {
                var that = this;
                globalThis.thatBeHalf = this;
                ...
                oRouter.getRoute("RouteBeHalf").attachPatternMatched(that._onRouteMatchedTimesheet, that);
            },

            _onRouteMatchedTimesheet: async function (oEvent) {
                var that = globalThis.thatBeHalf;
                ...
                if (!globalUserInfo.isAdmin) {
                    MessageBox.error(`Utente non abilitato alle funzionalitÃ  in BeHalf!`);
                    that.onUnauthorized();
                    return;
                }
            ...
            _bindView: async function () {
                ...
                if (beHalfUserInfo.DefaultEmailAddress) {

                    // Disattivo l'Extra Effort sugli Esterni
                    /*if (beHalfUserInfo.isExternal) {
                        that.getView().byId("idAddExtraEffort").setProperty("visible", false);
                        that.getView().byId("idButtonOvertimeBeHalf").setProperty("visible", false);
                        that.getView().byId("idButtonTimeOffBeHalf").setProperty("visible", false);

                    } else {
                        that.getView().byId("idAddExtraEffort").setProperty("visible", true);
                        that.getView().byId("idButtonOvertimeBeHalf").setProperty("visible", true);
                    //}*/
                    ...
                    //Inizializzo i dati
                    var StaffingListEmployee = this.getView().getModel("StaffingListEmployee");

                    if (StaffingListEmployee) {
                        StaffingListEmployee.destroy();
                    }

                    var TimesheetInfoEmployee = this.getView().getModel("TimesheetInfoEmployee");

                    if (TimesheetInfoEmployee) {
                        TimesheetInfoEmployee.destroy();
                    }

                    //await that.populateTableByDate({}, new Date(currentMonthValue));
                }
            },

            onBeHalfSelected: async function (oEvent) {
                var that = globalThis.thatBeHalf;
                that._pBusyDialog.then(async function (oBusyDialog) {
                    oBusyDialog.open();

                    var beHalfEmail = that.getView().byId('idComboBoxBeHalf').getSelectedKey();
                    if (beHalfEmail) {
                        var filterUser = `DefaultEmailAddress eq '${beHalfEmail}'`;
                        beHalfUserInfo = await that.getUserInfo(filterUser);

                        that.getView().byId("idUserLabel").setText(beHalfUserInfo.PersonFullName);

                        that._bindView();
                    } else {
                        MessageBox.error(`Selezionare un utente valido!`, {
            ...
            getStaffingListModel: async function (sDate) {
                ...
                let WorkAssignments = beHalfUserInfo.WorkAssignments.filter(personWA =>
            ...
            getTimesheetEmployee: async function (sDate) {
                ...
                var oWAFilter = this._createFilterFromArrayNew(beHalfUserInfo.WorkAssignments, "PersonWorkAgreement", "PersonWorkAgreement");
            ...
            loadTimesheetView: function (oEvent) {
                var selectedMenu = oEvent.getSource().getKey();
                if (selectedMenu === 'my') {
            ...
            populateTableByDate: async function (sTimesheetArray, sDate) {
                ...
                var freezeTimesheet = await this.checkFreezeTimesheet(beHalfUserInfo.isExternal, sDate);
                ...
                for (var sWBSElement in oDataByWBSElement) {
                    if (oDataByWBSElement.hasOwnProperty(sWBSElement)) {
                        ...
                        let matchingWA = beHalfUserInfo.WorkAssignments.find(el =>
                        ...
                        /*if (beHalfUserInfo.isExternal) {
                            matchingWA = beHalfUserInfo.WorkAssignments.find(el =>
                                el.PersonWorkAgreement === aTimeSheetEntriesForWBSElement[0].PersonWorkAgreement);
                        ...
                        for (var i = 1; i <= daysInMonth; i++) {
                            ...
                            if ((numeroGiorno === 0 || numeroGiorno === 6) || foundHoliday ||
                                ...
                                dateOnly < beHalfUserInfo.ValidFrom || dateOnly > beHalfUserInfo.ValidTo) {
            ...
            onBackToList: function () {
                ...
                // Torno alla schermata di selezione
            ...
            addProject: async function (oEvent) {
                ...
                var freezeTimesheet = await this.checkFreezeTimesheet(beHalfUserInfo.isExternal, currentMonthValue);
                if (freezeTimesheet) {
                ...
                if (selectedItem) {
                    ...
                    if (beHalfUserInfo.isExternal) {
                        ...
                        if (!matchingWA || matchingWA === null || matchingWA === undefined) {
                            ...
                            MessageBox.error(`La risorsa non ha un WorkAgreement relativo alla company del progetto selezionato (${prjCompanyCode[0].CompanyCode} - ${prjCompanyCode[0].CompanyCodeName})`);
                            return;
                        }*/
            ...
            /*onPopoverAfterOpen: function (oEvent) {
                this.getView().byId("popupTimePicker").focus();
            }*/
            ...
            onSelectedProject: async function () {
                ...
                if (!beHalfUserInfo.isExternal && prjBillingTypeCheck) {
                    that.getView().byId("idAddExtraEffort").setProperty("visible", true);
                } else {
                    that.getView().byId("idAddExtraEffort").setProperty("visible", false);
                }
                    ...
                    if (submitData.length > 0) {
                        ...
                        if (beHalfUserInfo.isExternal) {
                            ...
                            if (aArrayToCheck && aArrayToCheck.length > 0) {
                                var checkResult = await that.checkExternalUserPO(beHalfUserInfo.Person, aArrayToCheck);
            ...
            onSubmitTS: async function (oEvent) {
                ...
                that._pBusyDialog.then(async function (oBusyDialog) {
                    ...
                    let oModel = that.getView().getModel("TimesheetInfoEmployee");
                    let TimesheetInfoEmployee = oModel.oData;

                    //var firstDayOfMonthYear = new Date(currentMonthValue.getFullYear(), currentMonthValue.getMonth(), 1);
                    //var lastDayOfMonthYear = new Date(currentMonthValue.getFullYear(), currentMonthValue.getMonth() + 1, 0);
            ...
            saveInputData: async function (TimesheetInfoEmployee, aItems, TimesheetSingleCell, TimesheetSingleCellBackup, buttonCell) {
                ...
                var dailyWorkingHours = 8;
                let matchingWA = beHalfUserInfo.WorkAssignments.find(personWA =>
                    lastDayOfMonth >= (new Date(personWA.WorkAssignmentStartDate)) &&
                    firstDayOfMonth <= (new Date(personWA.WorkAssignmentEndDate)));
            ...
            // Gestione Overtime
            onCreateOvertimeRequest: function (oEvent) {
                var requestMode = 'createBeHalf';
                var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
                oRouter.navTo("RouteOvertimeRequest", {
                    requestMode: requestMode
                }, false);
            },

            // Gestione TimeOff
            onCreateTimeOffRequest: function (oEvent) {
                var requestMode = 'createBeHalf';
                var oRouter = sap.ui.core.UIComponent.getRouterFor(this);
                oRouter.navTo("RouteTimeOffRequest", {
                    requestMode: requestMode
                }, false);
            },
        });
